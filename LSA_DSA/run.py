...
CLIP_MIN = -0.5
CLIP_MAX = 0.5
# Set your local "google_drive_path" or the path that you uploaded the code in it
...
if __name__ == "__main__":
    ...
    if args.d == "fashion_mnist":
        ...
        model=load_model(str(Your_google_drive_path)+"/sadl11/model/model_fashion_mnist_LeNet4.h5")
        # # # You can select some layers you want to test.
        # # You can select some layers you want to test.
        layer_names = list(np.load(str(Your_google_drive_path)+"/sadl11/layer_names.npy"))
    if args.d == "mnist":
       ...
       # # You can select some layers you want to test.
        ...
        model= load_model(str(Your_google_drive_path)+"/sadl11/model/model_Cov.h5")
        # # You can select some layers you want to test.
        layer_names = list(np.load(str(Your_google_drive_path)+"/sadl11/layer_names.npy"))
    ...
    if args.d == "SVHN":
        ...
        model= load_model(str(Your_google_drive_path)+"/sadl11/model/model_SVHN_LeNet5.h5")
        # # You can select some layers you want to test.
        layer_names = list(np.load(str(Your_google_drive_path)+"sadl11/layer_names.npy"))
    ...
    if args.lsa:
      x_testLSC=np.load(str(Your_google_drive_path)+"/sadl11/tmp/x_tcovlsc.npy")
      ...
      np.save(str(Your_google_drive_path)+"/sadl11/tmp/test_cov.npy",test_cov1)
      print("args.upper_bound, args.n_bucket", args.upper_bound, args.n_bucket)
      print(infog("{} LSC coverage: ".format("test") + str(test_cov1)))
        
    if args.dsa:
        x_testDSC=np.load(str(Your_google_drive_path)+"/sadl11/tmp/x_tcovdsc.npy")
        ...
        np.save(str(Your_google_drive_path)+"/sadl11/tmp/DSC_cov.npy",test_cov)
